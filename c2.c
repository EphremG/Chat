...
int main(int argc, char **argv){
	...
	short host_port = 2222; 	/* It's the port of host machine */
	char *remote_ip = "127.0.0.1"; /* Ip address of remote machine*/
	short remote_port = 1111; 	/* port of remote machine */
	...
	z = bind(host_sock, (struct sockaddr *)&host_add, sizeof(host_add));
        ...
        puts("Waiting for the First Msg");
	...
	while(1){
		...
		if (strcmp("A", data) != 0) {
			...
			printf("You: ");
			bzero(datagram2, 5120);
			fgets(datagram2, 5120, stdin);
			...
			char* cipherText = Encipher(datagram2, 3);
...
//calculate crc
//if crc is not correct discard this message
if (strcmp(getMD5(z, buffer), crc_backup_buffer) != 0) {
printf("CRC Validation failed\n");
return 0;
}
...
//don't need crc anymore
//only need following:
hdr->pkLast = (unsigned char)buffer[38]; hdr->pkLast <<= 8;
hdr->pkLast += (unsigned char)buffer[39];

hdr->order = (unsigned char)buffer[34]; hdr->order <<= 8;
hdr->order += (unsigned char)buffer[35]; hdr->order <<= 8;
hdr->order += (unsigned char)buffer[36]; hdr->order <<= 8;
hdr->order += (unsigned char)buffer[37];

hdr->sizeOf = (unsigned char)buffer[40]; hdr->sizeOf <<= 8;
hdr->sizeOf += (unsigned char)buffer[41];

hdr->chunk = (char*)malloc(hdr->sizeOf + 1);
for (j = 0; j < hdr->sizeOf; j++)
hdr->chunk[j] = buffer[50 + j];
...
//decrypt and return when all received
if (r == pktlast) {
char *data = (char*)malloc(1 + totalSize);	//allocate enough memory
for (i = 0,k = 0; i < pktlast; i++) {
//for each segment
for (j = 0; j < seg_table[i]->sizeOf; j++) {
//copy all data of chunk
...
void prepareDatagram(char *data) {
//prepare following: pktLast, msgID
short pktLast;
int msgId;
struct header *hdr;
...
pktLast = ceil((double)dataSize / (double)MIN_LEN);
//calculate msgID
srand(time(0));
msgId = rand();
...
//pktchunk in loop
hdr->chunk = malloc(hdr->sizeOf + 1);
memcpy(hdr->chunk, &data[i*MIN_LEN],hdr->sizeOf);
hdr->chunk[hdr->sizeOf] = '\0';
...
buffer[34] = hdr->order >> 24;
buffer[35] = hdr->order >> 16;
buffer[36] = hdr->order >> 8;
buffer[37] = hdr->order;

buffer[38] = hdr->pkLast >> 8;
buffer[39] = hdr->pkLast;

buffer[40] = hdr->sizeOf >> 8;
buffer[41] = hdr->sizeOf;

buffer[42] = hdr->pktId >> 24;
buffer[43] = hdr->pktId >> 16;
buffer[44] = hdr->pktId >> 8;
buffer[45] = hdr->pktId;

buffer[46] = hdr->msgId >> 24;
buffer[47] = hdr->msgId >> 16;
buffer[48] = hdr->msgId >> 8;
buffer[49] = hdr->msgId;
